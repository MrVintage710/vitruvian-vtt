...
pub trait RulesItem : Sized {
    ...
    fn id(&self) -> Option<&RulesItemIdentifier>;
    ...
    fn values(&self) -> Vec<SimpleExpr> {
        Self::Table::list_columns().into_iter().map(|col| self.map_value(col)).collect()
    }
    ...
    fn get(id : impl Into<RulesItemIdentifier>, connection : &Connection) -> VitruvianRulesEngineResult<Self> {
        let id : RulesItemIdentifier = id.into();
        let id = id.id();
        ...
        let (sql, values) = Query::select()
            ...
            .and_where(Expr::col(Self::Table::id_variant()).eq(&id))
            .limit(1)
    ...
    fn insert(&self, connection : &Connection) -> VitruvianRulesEngineResult<()> {
        if self.id().is_none() { return Err(VitruvianRulesEngineError::InsertRuleItemError("There is no id associated with item.".to_string()))}
    ...
    fn update(&self, connection : &Connection) -> VitruvianRulesEngineResult<()> {
        ...
        let (sql, values) = Query::update()
            .table(Self::Table::table_variant())
            .values(self.map_values())
            .and_where(Expr::col(Self::Table::id_variant()).eq(id.id()))
            .build_rusqlite(SqliteQueryBuilder)
        ;
...
pub trait ItemTable: Iden + Sized + Copy {
